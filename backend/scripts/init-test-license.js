...
async function generateTestLicense() {
  try {
    ...
    if (existingActiveLicense) {
      console.log(`✅ Licence de test active existante : ${existingActiveLicense.licenseKey}`);
    ...
    // 4. Generer une nouvelle licence de test
    ...
    const testLicense = await prisma.license.create({
      data: {
        ...
        metadata: {
          type: 'test',
          description: 'Licence de test automatique - 24h',
          autoGenerated: true,
    ...
    console.log(`   🔑 Clé : ${testLicense.licenseKey}`);
    ...
    console.log(`   👥 Utilisateurs max : ${testLicense.maxUsers}`);
    ...
    console.log(`   Clé à copier : ${testLicense.licenseKey}`);
...
async function showTestLicenseStatus() {
  try {
    ...
    testClient.licenses.forEach((license, index) => {
      const isActive = license.isActive && license.expiresAt > new Date();
      const status = isActive ? '✅ ACTIVE' : '❌ EXPIRÉE';
      const timeLeft = isActive ? 
        Math.ceil((license.expiresAt - new Date()) / (1000 * 60 * 60)) + 'h restantes' :
        'Expirée';
      
      console.log(`   ${index + 1}. ${status} - ${license.licenseKey}`);
      console.log(`      📅 ${license.createdAt.toLocaleString()} -> ${license.expiresAt.toLocaleString()}`);
